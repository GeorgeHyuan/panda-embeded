 ...
 * @LastEditTime: 2019-10-11 11:31:33
 */
;(function() {
  ...
  function handleTitleCreating(editor, params) {
    ...
    if (inspector.type === 'symbol' && oldTitle === 'TitleBasic') {
      ...
      row.visible = function(inspector) {
        ...
        return (
          data.getClassName() === 'ht.Node' &&
          data.getImage() === 'symbols/basic/skew-image.json'
        )
      }
    } else if (inspector.type === 'comp' && title === 'TitleDash') {
      inspector.addTitle('虚线')
      inspector.addCustomProperty({
        property: 'dashColor',
        name: '虚线颜色',
        valueType: 'Color',
        ...
        extraInfo: {
          onPropertyChanged: function(event, inspector) {
            console.log(event, inspector)
            event.data.s('shape.dash.color', event.newValue)
          } // 默认为空，可根据属性变化更新属性页
        }
      })
    }
  ...
  function addPropertiesToSymbol(inspector) {
    ...
    var onClicked = function() {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        parent.setIcon('custom/symbols/basic/ht.json')
