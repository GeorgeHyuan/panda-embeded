!function(a, b, c, d) {
    ...
    ,
    g.prototype.addNode = function (identifiers, options) {
	    this.forEachIdentifier(identifiers, options, $.proxy(function (node, options) { 
            if(options.node) {
                this.setAddNode(node, options);
            }else{
                node.icon = 'glyphicon glyphicon-euro'
            }
	    }, this));
	    this.setInitialStates({ nodes: this.tree }, 0);
	    this.render();
    },
    g.prototype.setAddNode = function (node, options) {
	    if (node.nodes == null) node.nodes = [];
	    if (options.node) {
            node.icon = ''
	        node.nodes.push.apply(node.nodes,options.node)
           }
	},
